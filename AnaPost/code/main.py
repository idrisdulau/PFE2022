...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help='if true, the top k accuracy by category  will be displayed')
    parser.add_argument(
        '--display_recall_precision',
        type=str2bool,
        default=False,
        help='if true, recall and precision by category  will be displayed')
...
class Processor():
    ...
    def save_checkpoint(self, path, filename, epoch):
        ...
        try:
            ...
            print("[MOD] - In save_checkpoint() method :")
    ...
    def json_params(self, savedir):
        try:
            ...
            print("[MOD] - In json_params() method :")
    ...
    def load_model(self):
        ...
        print("self.arg.weights:",self.arg.weights)
        if self.arg.weights:
    ...
    def train(self, epoch, save_model=False):
        ...
        print("\narg.accumulating_gradients:",arg.accumulating_gradients)
        if (not arg.accumulating_gradients):
            for batch_idx, (data, label, name) in enumerate(loader):
                ...
                if (batch_idx + 1) % 200 == 0:
                    ...
                    stats_train = 'Training: Epoch [{}/{}], Step [{}], Loss: {}, Training Accuracy: {}'.format(epoch,self.arg.num_epoch,batch_idx,loss.item(),acc)
                    print('\n' + stats_train)
            ...
            for batch_idx, (data, label, name) in enumerate(loader):
                ...
                conf_matrix_train += confusion_matrix(label.cpu(), predictions.cpu(), labels=np.arange(self.arg.model_args['num_class']))
                np.save("./checkpoints/cm/cm_" + str(epoch), conf_matrix_train)
    ...
    def test(self, epoch, save_score=True, loader_name=['test']):
        self.model.eval()
        print("\nMODE = [TEST]")
        self.print_log('Eval epoch (test): {}'.format(epoch + 1))
        val_correct = 0
        val_total = 0
        conf_matrix_test = 0
        ...
        for ln in loader_name:
            ...
            print("[Test] - Pred labels: ", predictions)
            print("[Test] - Real labels: ", label)
            print("[Test] - Total samples: ", val_total)
            ...
            for i in range(0, self.arg.model_args['num_class']):
                if class_total[i] != 0:
                    print("class_total i:",class_total[i])
            ...
            for i in range (self.arg.num_epoch):
                cm = np.load("./checkpoints/cm/cm_"+str(i)+".npy")
                ...
                if (preci + sensi > 0):
                    f1score = (preci * sensi)/(preci + sensi)
                else:
                    f1score = 0

                epoch_plt.append(i)
                accuracy_plt.append(acc)
                f1score_plt.append(f1score)


            cm = np.load("./checkpoints/cm/cm_"+str((self.arg.num_epoch)-1)+".npy")
    ...
    def val(self, epoch, save_score=False, loader_name=['val']):
        self.model.eval()
        print("\nMODE = [VAL]")
        self.print_log('Eval epoch (val): {}'.format(epoch + 1))
        val_correct = 0
        conf_matrix_val = 0
        val_total = 0
    ...
    def start(self):
        ...
        print("Params:",pytorch_total_params,"; Layer params:",layer_params)
        patience = 50
        patient_counter = 0
        print("num_class:",self.arg.model_args['num_class'])                    
        print("num_point:",self.arg.model_args['num_point']) 

        print("self.arg.training = ",self.arg.training)
        if not self.arg.training:
            print("self.arg.training = False; go in test mode")
            self.test(epoch=0, save_score=self.arg.save_score, loader_name=['test'])
        
        print("self.arg.phase = ",self.arg.phase)
        if self.arg.phase == 'train':
            ...
            try:
                ...
                for k, v in state.items():
                        if 'module' not in k:
                                ...
                                k.replace('features.module.', 'module.features.')
                        new_state[k]=v
                        weights = new_state
                diff = list(set(state.keys()).difference(set(weights.keys())))
                print('Can not find these weights:')
